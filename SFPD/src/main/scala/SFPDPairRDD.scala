//--------------------------------------------------------------------------------
// MapR DEV 361 - Apache Spark Essentials
...
object SFPDPairRDD {
  ...
  def main(args: Array[String]) {
    ...
    val sfpdRDD = sc.textFile("/user/wynadmin/sfpd.csv").map(line => line.split(","))
    ...
    val top5Adds = sfpdRDD
      .map(incident => (incident(Address), 1))
      .reduceByKey(_ + _)
      .map(x => (x._2, x._1))
      .sortByKey(false)
      .take(5)
    ...
    val top3Cat = sfpdRDD
      .map(incident => (incident(Category), 1))
      .reduceByKey(_ + _)
      .map(x => (x._2, x._1))
      .sortByKey(false)
      .take(3)
    ...
    val catAdd = sc.textFile("/user/wynadmin/J_AddCat.csv").map(x => x.split(",")).map(x => (x(1), x(0)))
    val distAdd = sc.textFile("/user/wynadmin/J_AddDist.csv").map(x => x.split(",")).map(x => (x(1), x(0)))
