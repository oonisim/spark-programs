...
object NQueen {
  ...
  def test(args: Array[String]): Unit = {
    ...
    def queensAtFirst: Set[List[Int]] = {
      val placements = for {
        col <- 0 until n
      } yield {
        List(col)
    ...
    val result = queensAtFirst.map { x => placeQueensAt(1, Set(x)) }
    //val result = placeQueens(0, queensAtFirst).collect()
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result = initial.flatMap(x => placeQueensAt(1, Set(x)))
    result.saveAsTextFile("hdfs:/user/wynadmin/nqueen")
    
    println("-------------------------------- print result. Size %d".format(result.collect().size))
    //result.foreach { x => println(x) }
